...
open SEQ, ">${dir}seq_open_rc_si".$POS."_".$PROB_L.".txt" or die "Can't open output file ${dir}seq.txt";
#open SEQ_bg, ">${dir}seq_bg_open_rc_si6.txt" or die "Can't open output file ${dir}seq.txt";
open LABEL, ">${dir}label_open_rc_si".$POS."_".$PROB_L.".txt" or die "Can't open output file ${dir}label.txt";
open SAM, ">${dir}sample_open_rc_si".$POS."_".$PROB_L.".txt" or die "Can't open output file ${dir}sample.txt";
open MID, ">${dir}mid_open_rc_si".$POS."_".$PROB_L.".txt" or die "Can't open output file ${dir}mid.txt";
...
while(<CHROM>){
	...
	while($genome_pos<$oe){
		...
		if($chrom and ($chrom==$ochr and $pos==$genome_pos)){
			...
			$spikein_pos=3;
			while($spikein_pos>0){
				${$seq}[$POS+$spikein_pos]="A", if(${$seq}[500] eq 'C' and $mut==0 and rand()>$PROB);
				$spikein_pos--;
			}
